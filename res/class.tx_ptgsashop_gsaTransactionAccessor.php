...
class tx_ptgsashop_gsaTransactionAccessor extends tx_ptgsasocket_gsaDbAccessor implements tx_pttools_iSingleton {
    ...
    public function insertShopOrderTransactionDocPosition($parentDocId, $posCounter, $deliveryObj, $predecessorDocUid, 
        ...
        $tmpDataArr['gsaArticleDataArr'] = tx_ptgsashop_articleAccessor::getInstance()->selectCompleteArticleData($articleObj->get_id());
            #### TODO: this is a temporary solution - wenn Bedeutung/Funktion der einz. GSA-Artikel-Eigenschaften gekl�rt ist und diese korrekt verwendet werden, diese hier nicht mehr aus DB lesen, sondern aus Artikel-Properties!
    ...
    /**
     * Inserts a data carrier exchange (ERP: "DTA/Datentr�geraustausch") record into the GSA DB table 'px_DTABUCH' and returns the inserted record's UID.
     * 
     ...
     * @param   string      GSA transaction document type abbreviation (e.g. 'RE' for "Rechnung" (=invoice), 'AU' for "Auftragsbest�tigung" (=order confirmation), ...)
